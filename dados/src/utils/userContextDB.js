...
class UserContextDB {
  ...
  /**
   * Atualiza o relacionamento com Toji
   */
  updateRelationship(userId, campo, valor) {
    const context = this.getUserContext(userId);
    
    if (context.relacionamento_toji.hasOwnProperty(campo)) {
      context.relacionamento_toji[campo] = valor;
      context.ultima_atualizacao = getBrazilDateTime();
      this.saveDatabase();
    }
  }
  ...
  addSpecialMemory(userId, memoria) {
    ...
    context.relacionamento_toji.memorias_especiais.push(novaMemoria);
    
    // Manter apenas as 30 memórias mais especiais
    if (context.relacionamento_toji.memorias_especiais.length > 30) {
      context.relacionamento_toji.memorias_especiais = 
        context.relacionamento_toji.memorias_especiais.slice(-30);
    }
  ...
  updateMemory(userId, tipo, valorAntigo, valorNovo) {
    ...
    switch (tipoNormalizado) {
      ...
      case 'memória':
        const indexMemoria = context.relacionamento_toji.memorias_especiais.findIndex(
          m => m.texto === valorAntigo
        );
        if (indexMemoria !== -1) {
          context.relacionamento_toji.memorias_especiais[indexMemoria].texto = valorNovo;
          context.relacionamento_toji.memorias_especiais[indexMemoria].data = getBrazilDateTime();
          atualizado = true;
        }
        break;
  ...
  deleteMemory(userId, tipo, valor) {
    ...
    switch (tipoNormalizado) {
      ...
      case 'memória':
        ...
        if (indexMemoria !== -1) {
          context.relacionamento_toji.memorias_especiais.splice(indexMemoria, 1);
          removido = true;
        }
        break;
  ...
  getUserContextSummary(userId) {
    ...
    const summary = {
      ...
      nivel_intimidade: context.relacionamento_toji.nivel_intimidade,
      ...
      memorias_especiais: context.relacionamento_toji.memorias_especiais.slice(-5).map(m => m.texto).join('\n- ') || 'Nenhuma'
    };
